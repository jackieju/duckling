...
ruleCycleLastNextN2 = Rule
  ...
  , pattern =
    [ regex "((the )?(last|past|recent|previous)|((the )?next|(the )?coming|(the )?following))"
    , Predicate $ isIntegerBetween 1 9999
    , dimension TimeGrain
    ]
  , prod = \tokens -> case tokens of
      (Token RegexMatch (GroupMatch (match:_)):token:Token TimeGrain grain:_) -> do
        n <- getIntValue token
        tt . cycleN True grain $ if match == "last" then - n else if match == "past" then - n else if match == "recent" then - n else if match == "previous" then - n else  n
      _ -> Nothing
  }
